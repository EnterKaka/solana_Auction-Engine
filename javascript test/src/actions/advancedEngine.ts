...
import * as auction_data from './auction/auction_v3'
...
export class advancedEngine{
	...
	setupAuction = async (price:number,tick:number,instant_sale_price:number,name:number[],reward:number) =>{
		...
		await auction_api.createAuction(
			this.connection,
			this.auctionCreator,
			this.resource,
	...
	createAuction = async(price:number,tick:number,instant_sale_price:number,name:number[],reward:number) =>{
		await this.setupAuction(price, tick, instant_sale_price, name, reward)
	}
