...
export default class AuctionPage extends Component<any,any> {
	...
	constructor(props : any){
		...
		this.state={
			auctionName : "",
			instantSalePrice : 200,
			minprice : 100,
			tick : 10,
			reward : 4,
			existInstantSalePrice : false,
			validAuction : false,
	...
	createAuction = async () => {
		try{
			let name : number[] = []
			for(let i=0;i<32;i++)
				name.push(this.state.auctionName.charCodeAt(i))
			await this.engine.createAuction(this.state.minprice,this.state.tick,this.state.existInstantSalePrice ? this.state.instantSalePrice : 0, name ,this.state.reward)
	...
	changeAmount = async()=>{
		if(this.auction.bidState.bids!=undefined && this.auction.bidState.bids.length!=0){
			for(let i=0;i<4;i++){
				if(this.amounts[i].place != 0){
					this.amounts[i].own=this.amounts[i].own
					this.pot=Number(this.amounts[i].place)
					this.amounts[i].place=this.auction.bidState.bids[0].amount
				}
			}
		}	
	}
		...
		return <div className="container-fluid mt-4">
			<div className="row">
				<div className="col-lg-6">
					<form>
						<div className="input-group mb-3">
							...
							<input name="auctionName" type="text" className="form-control" onChange={this.changeInputValue} value={this.state.auctionName}/>
						</div>
						<div className="input-group mb-3">
							<div className="input-group-prepend">
					...
					</form>
					{
						...
						<>
							<h5>Auction Data:</h5>
							<p>Auction Name   :  {this.auction_extended.name}</p>
							<p>Auction State : {this.auction.state == 2 ? "Ended" : "Started"}</p>
				...
				<div className="col-lg-6">
					...
					<Bidder idx='2'  title="Person3" own={this.amounts[2].own} place={this.amounts[2].place} placeCallback={this.placeBid} cancelCallback={this.cancelBid}/>
					<Bidder idx='3' title="Person4" own={this.amounts[3].own} place={this.amounts[3].place} placeCallback={this.placeBid} cancelCallback={this.cancelBid}/>			
				</div>
				}
			</div>
		</div>
